...
function lightPointEffect.getshader()
    ...
    local lightShader = love.graphics.newShader(string.format([[
        ...
        // 点光源结构体（新增 blurSize 控制边缘模糊大小）
        struct PointLight {
            vec2 position;   // 光源位置
            vec3 color;      // 光源颜色
            float radius;    // 方形光照边长的一半（原半径）
            float intensity; // 光照强度
            float falloff;   // 衰减系数
            bool actived;    // 是否激活
            float blurSize;  // 边缘模糊大小（像素单位）
        };
        ...
        vec4 effect(vec4 baseColor, Image tex, vec2 texCoord, vec2 screenCoord) {
            ...
            for (int i = 0; i < %d; i++) {
                ...
                // 计算像素到光源的「切比雪夫距离」（方形距离）
                vec2 lightPos = lights[i].position;
                vec2 delta = abs(screenCoord - lightPos); // 方形偏移
                float maxDelta = max(delta.x, delta.y);   // 方形距离（核心）
    
                // 超出方形光照范围则跳过
                if (maxDelta > lights[i].radius) continue;
    
                // 计算边缘模糊（核心修改）
                float blurSize = lights[i].blurSize;
                // 归一化距离（0~1）
                float normalizedDist = maxDelta / lights[i].radius;
                // 模糊过渡：在 [1 - blurRatio, 1] 区间内平滑衰减
                float blurRatio = blurSize / lights[i].radius;
                float smoothEdge = smoothstep(1.0, 1.0 - blurRatio, normalizedDist);
    
                // 计算光照衰减（保留原衰减逻辑 + 模糊）
                float attenuation = 1.0 / (1.0 + lights[i].falloff * normalizedDist * normalizedDist);
                attenuation = smoothEdge * attenuation; // 融合模糊效果
